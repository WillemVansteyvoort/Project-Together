...
export default class SignupFacebook extends Component {

    constructor(props) {
        super(props)
        this.state = {

            name: window.Laravel.facebook.name,
    ...
    //step 1
    buttonStep1() {
        if(this.state.name_errors || this.state.lastname_errors || this.state.email_errors || this.state.name.length <= 2 || this.state.lastname.length <= 2 || this.state.email.length <= 6) {
...
if (document.getElementById('signup-facebook')) {
    ReactDOM.render(<SignupFacebook />, document.getElementById('signup-facebook'));
}
